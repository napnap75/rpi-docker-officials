...
if [ "$2" == "" ]; then
  cd influxdata-docker/chronograf/$1
  patch Dockerfile ../../../Dockerfile-$1.patch
else
  cd influxdata-docker/chronograf/$1/$2
...
# Build the image
if [ "$2" == "" ]; then
  docker build -t napnap75/rpi-chronograf:$1 .
else
  docker build -t napnap75/rpi-chronograf:$1-$2 .
fi
docker images
